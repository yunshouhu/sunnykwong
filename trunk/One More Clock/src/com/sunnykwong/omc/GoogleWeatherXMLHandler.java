package com.sunnykwong.omc;
	...
	static public void updateWeather() {
        ...
        Thread t = new Thread () {
            public void run() {
	            try {
                    ...
                    request.setURI(new URI("http://www.google.com/ig/api?weather=77584"));
	    ...
	    public void startDocument () {
	    	if (OMC.DEBUG) Log.i(OMC.OMCSHORT + "Weather","Start Building JSON.");
	    }
	    ...
	    public void startElement(String namespaceURI, String localName,
	    	...
	    	if (!tree.isEmpty()) {
		    	...
		    	if (tree.peek()[0].equals("forecast_information") || tree.peek()[0].equals("current_conditions")) {
		    		if (OMC.DEBUG) Log.i(OMC.OMCSHORT + "Weather","Reading " + tree.peek()[0] + " - " + localName);
		    		try {
		    		...
		    		if (OMC.DEBUG) Log.i(OMC.OMCSHORT + "Weather","Reading " + tree.peek()[0] + " - " + localName);
		    		if (jsonOneDayForecast == null) jsonOneDayForecast = new JSONObject();
			    	try {
	    ...
	    public void endElement (String uri, String name, String qName) {
	    	...
	    	if (OMC.DEBUG) Log.i(OMC.OMCSHORT + "Weather","EndElement."+ name);
	    	if (name.equals("forecast_conditions")) {
	    		try {
		    		jsonWeather.getJSONArray("zzforecast_conditions").put(jsonOneDayForecast);
			    	jsonOneDayForecast = null;
	    		} catch (JSONException e) {
	    			e.printStackTrace();
	    		}
	    ...
	    public void endDocument () {
	    	if (OMC.DEBUG) Log.i(OMC.OMCSHORT + "Weather",jsonWeather.toString());
	    	if (OMC.DEBUG) Log.i(OMC.OMCSHORT + "Weather","End Document.");
			...
			tree = null;
			OMC.PREFS.edit()
			.putString("ompc_weatherjson", jsonWeather.toString())
			.commit();
	    }
